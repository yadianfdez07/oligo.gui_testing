...
using System.Collections; //chapter 12
...
namespace GUITestLibrary
{
	...
	public class GUITestActions
	{
		...
		public static ArrayList FindMenuClickPathByID(int parentWinhandle, int menuID)
		{
			...
			{
				FindMenuTextAndID(wHnd, i, ref thisPath, ref handleFound);
				if (!handleLst.Contains(handleFound))
				{
					handleLst.Add(handleFound);
					menuPath.Insert(0, thisPath);
				}
				if (handleFound == wHnd)
				{
					break;
		...
		//private static void FindMenuWindow(int parentHndl, ref string parentText, ref int menuWin, ref string winText)
		private static void FindMenuWindow(ref int menuWin, ref string winText, ref string parentText)
		{
			
			string clsName = "#32768";
			
			FindGUILike(ref menuWin, 0, ref winText, ref clsName, ref parentText);
			
			//StringBuilder altWin = new StringBuilder(winText + "C");
			//SetWindowText(menuWin, altWin);
			//winText = altWin.ToString();
			
		}

//		private static string FindParentText(int parentHndl)
//		{
//			StringBuilder parentText = new StringBuilder();
//			GetWindowText(parentHndl, parentText, 128);
//			return parentText.ToString();
//		}

		private static void ClickMenuItem(int mWnd, int hMenu, int Pos)
		{
			RECT rct = new RECT();
			POINTAPI pnt;
			
			int ret = GetMenuItemRect(mWnd, hMenu, Pos, ref rct);
			
			if (ret == 0)
			{
				return;
			}
			pnt.x = (rct.Left + rct.Right) / 2;
			pnt.y = (rct.Top + rct.Bottom) / 2;
			PixelXYToMickeyXY(ref pnt.x, ref pnt.y);
			
			mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE, pnt.x, pnt.y, 0, GetMessageExtraInfo());
			ClickMouse(MonkeyButtons.btcLeft, pnt.x, pnt.y, 0, 0);
			//ClickMouse(pnt);
		}
//		private static void ClickMouse(POINTAPI p)
//		{
//			mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE, p.x, p.y, 0, GetMessageExtraInfo());
//			//'Mouse Down
